...
public class YagenHandler
{
    ...
    public virtual async Task<int> Execute(YagenArguments arguments)
    {
        try
        {
            return await this.ExecuteImpl(arguments);
        }
        catch (Exception e)
        {
    ...
    protected virtual async Task<int> ExecuteImpl(YagenArguments arguments)
    {
        ...
        return await this.ExecuteGoals(arguments, manifest);
    }
    ...
    /// <returns></returns>
    protected virtual async Task<int> ExecuteGoals(YagenArguments arguments, YagenManifest manifest)
    {
        // the set of requested goals
        var requestedGoals = arguments.Goals?.ToList() ?? [];

        // all the defined goals
        var definedGoals = manifest.Goals?.Select(goal => goal.Name).ToList() ?? [];
    ...
    private static YagenGoal MapGoal(YagenArguments args, YagenManifest manifest, YagenGoalManifest goal)
    {
        ...
        {
            Name = goal.Name,
            Layers = goal.Layers?.Select(path => new DirectoryInfo(Path.GetFullPath(path, args.Input.FullName))).ToList() ?? [],
            EnvFiles = goal.EnvFiles?.Select(path => new FileInfo(Path.GetFullPath(path, args.Input.FullName))).ToList() ?? [],
            ValueFiles = goal.ValueFiles?.Select(path => new FileInfo(Path.GetFullPath(path, args.Input.FullName))).ToList() ?? [],
